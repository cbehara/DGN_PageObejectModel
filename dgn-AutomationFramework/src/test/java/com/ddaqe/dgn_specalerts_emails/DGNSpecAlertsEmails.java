...
public class DGNSpecAlertsEmails extends BaseTest {
	static Logger log = Logger.getLogger(DGNSpecAlertsEmails.class);
	...
	@Test(groups = {
			"Medium" }, dataProviderClass = DGNSpecAlertsEmailsDataProvider.class, dataProvider = "TCNonAdEmail", description = "[DGN] Display 'in progress' indicator on top of pagination controls while system loads the report ids (TC20998)")
	public void tc1357(String emailAddress, String password, String alternateEmailAddress, String optionalMsg) {
	...
	@Test(groups = {
			"Medium" }, dataProviderClass = DGNSpecAlertsEmailsDataProvider.class, dataProvider = "TCNonAdEmail", description = "[DGN] Display 'in progress' indicator on top of pagination controls while system loads the report ids (TC20998)")
	public void tc1356(String emailAddress, String password, String alternateEmailAddress, String optionalMsg) {
		HomePage homePage = loginAs(emailAddress, password);
		CompanyResultsPage companyResultPage = homePage.clickOnCompaniesLink();
		companyResultPage.clickOnFirstCompanyChkBox();
		companyResultPage.clickOnActionsDropdown();
