...
public class DGNProjectTrackingListEmails extends BaseTest {
	static Logger log = Logger.getLogger(DGNProjectTrackingListEmails.class);
	...
	@Test(groups = {
			"Medium" }, dataProviderClass = DGNProjectTrackingListEmailsDataProvider.class, dataProvider = "TCNonAdEmail", description = "[DGN] Display 'in progress' indicator on top of pagination controls while system loads the report ids (TC20998)")
	public void tc284(String emailAddress, String password, String alternateEmailAddress, String optionalMsg) {
	...
	@Test(groups = {
			"Medium" }, dataProviderClass = DGNProjectTrackingListEmailsDataProvider.class, dataProvider = "TCNonAdEmail", description = "[DGN] Display 'in progress' indicator on top of pagination controls while system loads the report ids (TC20998)")
	public void tc289(String emailAddress, String password, String alternateEmailAddress, String optionalMsg) {
		HomePage homePage = loginAs(emailAddress, password);
		CompanyResultsPage companyResultPage = homePage.clickOnCompaniesLink();
		companyResultPage.clickOnFirstCompanyChkBox();
		companyResultPage.clickOnActionsDropdown();
